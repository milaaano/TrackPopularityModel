...
- RESOLVED: `artist_fame_loo` was **replaced by `artists_listeners`** (external Last.fm signal). It is a leave-one-out target encoding derived from popularity (corr 0.85 vs 0.41), and is unavailable at serving time for a new upload — so it is excluded from every model input and survives only in `notebooks/` as a data-cleaning tool.
