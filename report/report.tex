...
Decomposing \emph{Why}}\vspace{-0.5em}}
\author{Milana Medetova}
\date{}
...
contribution. Along the way we document two instructive failures: a
...
This project therefore has two goals. The first is conventional --- fit the best
model we can, measured properly. The second is the actual product: split the
...
We replaced it with an external signal: total listener counts per artist pulled
...
We escalated model capacity deliberately, using each result to justify the next
step rather than sweeping architectures at random.
...
identical ($R^2 = 0.6009$ vs $0.5983$), which already says something useful.
...
the signature of \textbf{high bias}: the hypothesis class is too simple for the
data, and the fix is more capacity, not more regularisation.
...
linear model can express. It improves markedly (Figure~\ref{fig:mlp}): test
$R^2 = 0.6494$, MAE $7.285$, RMSE $10.540$. The gap between train $R^2 = 0.7284$
and test $0.6494$ indicates mild overfitting, but the headline is that
\textbf{non-linearity helps}. Given that \texttt{artists\_listeners} is the
dominant numeric predictor, this implies the fame-to-popularity relationship is
curved --- plausibly with saturating returns, where the difference between
10{,}000 and 100{,}000 listeners matters far more than between 5M and 10M.
...
We evaluated four ensembles (Figure~\ref{fig:trees}). \textbf{Random forests}
...
while every audio feature lands between 471 and 618. Context, not sound,
explains most of Spotify popularity.
...
could keep using familiar names. We rejected it. Spotify's features are
...
\textbf{Option B}, which we adopted, is to retrain the audio model directly on
...
LightGBM importances put genre and fame far ahead. The tempting conclusion ---
that audio does not matter --- is wrong.
